        ...
        <div className="content-Container">
            <form className="row ui form">
                <div className="two wide field">
                    ...
                    <input type="text" name="gymnast" placeholder="Gymnast Name" value={newGymnast} onChange={(e) => setNewGymnast(e.currentTarget.value)}/>
                </div>
                <button className="ui button" type="button" onClick={() => addGymnast()}>Add gymnast</button>
            </form>
            <div className="ui grid">
                {gymnasts.map((gym, index) => <GymnastsEntry key={index} gym={gym} apparatusClicked={apparatusClicked} deleteGymnast={deleteGymnast}/>)}
            </div>
        </div>
    )
};
